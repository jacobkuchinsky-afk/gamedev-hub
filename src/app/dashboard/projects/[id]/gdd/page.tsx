...
export default function GDDPage() {
  ...
  const handleExportFullGDD = useCallback(() => {
    const name = project?.name || "Untitled";
    const slug = name.toLowerCase().replace(/\s+/g, "-");
    const date = new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });
    const lines: string[] = [];

    lines.push(`# ${name} — Full Game Design Document`);
    lines.push("");
    lines.push(`> Generated by GameForge on ${date}`);
    lines.push("");

    const tocEntries: string[] = [];
    let tocIndex = 1;

    const filledSections = GDD_SECTIONS.filter((s) => s.fields.some((f) => data[f.key]?.trim()));
    if (filledSections.length > 0) filledSections.forEach((s) => { tocEntries.push(`${tocIndex++}. [${s.title}](#${s.id})`); });
    if (pillars.length > 0) tocEntries.push(`${tocIndex++}. [Design Pillars](#design-pillars)`);
    if (competitorResult) tocEntries.push(`${tocIndex++}. [Competitor Analysis](#competitor-analysis)`);
    if (gameLoopResult.trim()) tocEntries.push(`${tocIndex++}. [Game Loop Breakdown](#game-loop-breakdown)`);
    if (mechanicsResult.trim()) tocEntries.push(`${tocIndex++}. [Mechanics Deep Dive](#mechanics-deep-dive)`);
    if (tutorialResult.trim()) tocEntries.push(`${tocIndex++}. [Tutorial Flow](#tutorial-flow)`);
    if (scenePlannerResult.trim()) tocEntries.push(`${tocIndex++}. [Scene Plan](#scene-plan)`);
    if (enemies.length > 0) tocEntries.push(`${tocIndex++}. [Enemies](#enemies)`);
    if (powerUps.length > 0) tocEntries.push(`${tocIndex++}. [Power-Ups](#power-ups)`);
    if (achievements.length > 0) tocEntries.push(`${tocIndex++}. [Achievements](#achievements)`);
    if (codexEntries.length > 0) tocEntries.push(`${tocIndex++}. [Wiki / Codex](#wiki--codex)`);
    if (conceptDocResult.trim()) tocEntries.push(`${tocIndex++}. [Concept Document](#concept-document)`);

    lines.push("## Table of Contents");
    lines.push("");
    tocEntries.forEach((e) => lines.push(e));
    lines.push("");
    lines.push("---");
    lines.push("");
    ...
    if (pillars.length > 0) {
      lines.push("## Design Pillars {#design-pillars}");
      lines.push("");
      const sorted = [...pillars].sort((a, b) => b.priority - a.priority);
      sorted.forEach((p, i) => {
        lines.push(`### ${i + 1}. ${p.name} (Priority: ${p.priority}/5)`);
        lines.push("");
    ...
    if (competitorResult) {
      lines.push("## Competitor Analysis {#competitor-analysis}");
      lines.push("");
      if (competitorResult.usp) {
        lines.push(`**Unique Selling Proposition:** ${competitorResult.usp}`);
        lines.push("");
      }
      if (competitorResult.competitors.length > 0) {
        lines.push("| Competitor | Type | Strengths | Weaknesses | Our Differentiation |");
        lines.push("|---|---|---|---|---|");
        competitorResult.competitors.forEach((c) => {
          lines.push(`| ${c.name} | ${c.type} | ${c.strengths} | ${c.weaknesses} | ${c.differentiation} |`);
        });
        lines.push("");
      }
    }

    if (gameLoopResult.trim()) {
      lines.push("## Game Loop Breakdown {#game-loop-breakdown}");
      lines.push("");
      lines.push(gameLoopResult);
      lines.push("");
    }

    if (mechanicsResult.trim()) {
      lines.push("## Mechanics Deep Dive {#mechanics-deep-dive}");
      lines.push("");
      lines.push(mechanicsResult);
      lines.push("");
    }

    if (tutorialResult.trim()) {
      lines.push("## Tutorial Flow {#tutorial-flow}");
      lines.push("");
      lines.push(tutorialResult);
      lines.push("");
    }

    if (scenePlannerResult.trim()) {
      lines.push("## Scene Plan {#scene-plan}");
      lines.push("");
      lines.push(scenePlannerResult);
      lines.push("");
    }

    if (enemies.length > 0) {
      lines.push("## Enemies {#enemies}");
      lines.push("");
      enemies.forEach((e) => {
        lines.push(`### ${e.name}`);
        lines.push("");
        ...
        lines.push(`- **Attack Damage:** ${e.attackDamage}`);
        lines.push(`- **Speed:** ${e.speed}`);
        lines.push(`- **Special Ability:** ${e.specialAbility}`);
        lines.push(`- **Behavior Pattern:** ${e.behaviorPattern}`);
        lines.push(`- **Visual Description:** ${e.visualDescription}`);
        lines.push("");
      });
    }

    if (powerUps.length > 0) {
      lines.push("## Power-Ups {#power-ups}");
      lines.push("");
      powerUps.forEach((p) => {
        lines.push(`### ${p.icon} ${p.name} — *${p.rarity}*`);
        lines.push("");
        lines.push(`- **Effect:** ${p.effect}`);
        lines.push(`- **Duration:** ${p.duration}`);
        lines.push(`- **Cooldown:** ${p.cooldown}`);
        lines.push(`- **Visual Effect:** ${p.visualEffect}`);
        lines.push("");
      });
    }

    if (achievements.length > 0) {
      lines.push("## Achievements {#achievements}");
      lines.push("");
      lines.push("| Achievement | Description | Rarity | Condition |");
      lines.push("|---|---|---|---|");
      achievements.forEach((a) => {
        lines.push(`| ${a.name} | ${a.description} | ${a.rarity} | ${a.condition} |`);
      });
      lines.push("");
    }

    if (codexEntries.length > 0) {
      lines.push("## Wiki / Codex {#wiki--codex}");
      lines.push("");
    ...
    if (conceptDocResult.trim()) {
      lines.push("## Concept Document {#concept-document}");
      lines.push("");
      lines.push(conceptDocResult);
      lines.push("");
    }

    lines.push("---");
    lines.push("");
    lines.push(`*End of ${name} Game Design Document — GameForge*`);

    const blob = new Blob([lines.join("\n")], { type: "text/markdown" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `${slug}-gdd-full.md`;
    a.click();
    URL.revokeObjectURL(url);

    setFullExported(true);
    setTimeout(() => setFullExported(false), 2500);
  }, [project, data, pillars, competitorResult, gameLoopResult, mechanicsResult, tutorialResult, scenePlannerResult, enemies, powerUps, achievements, codexEntries, conceptDocResult]);
        ...
        <div>
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                onClick={handleExportFullGDD}
                className={`flex items-center gap-1.5 rounded-lg border px-3 py-2 text-sm font-medium transition-all ${
                  fullExported
                    ? "border-[#10B981]/50 bg-[#10B981]/10 text-[#10B981]"
                    : "border-[#F59E0B]/30 bg-[#F59E0B]/5 text-[#F59E0B] hover:border-[#F59E0B]/50 hover:bg-[#F59E0B]/10"
                }`}
              >
                {fullExported ? <CheckCircle2 className="h-3.5 w-3.5" /> : <FileText className="h-3.5 w-3.5" />}
                {fullExported ? "Exported!" : "Export Full GDD"}
              </button>
              <button
                onClick={handleReset}
